...
const GROQ_QUERY = `*[_type == "loan" && loanCategory == "regular"]{
  title,
...
async function fetchRegularLoans() {
  if (!container) return;
  ...
  try {
    ...
    if (!loans || loans.length === 0) {
      container.innerHTML =
        '<p>No regular loan products are available at this time. Please check back later.</p>';
      return;
    }
    ...
    container.innerHTML =
      '<p>Unable to load regular loan products at this time. Please try again later.</p>';
  }
}

document.addEventListener('DOMContentLoaded', fetchRegularLoans);
